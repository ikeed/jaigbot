...
def test_init_session_backend_persona():
    ...
    payload = {"sessionId": sid, "personaId": "Jasmine"}
...
def test_init_session_new_connection_replaces_active_connection():
    sid = "test-sid-active"
    conn1 = "conn-1"
    conn2 = "conn-2"
    MEMORY_STORE.pop(sid, None)
    ...
    # Second connection with DIFFERENT connectionId replaces the old active
    # connection because server-side connection ids change during reconnects.
