...
def test_init_session_force_takeover():
    sid = "test-sid-force"
    conn1 = "conn-1"
    conn2 = "conn-2"
    ...
    # Second connection (would normally be blocked)
    resp2 = client.post("/session", json={"sessionId": sid, "connectionId": conn2})
    assert resp2.json()["alreadyActive"] is True
    
    # Third connection with FORCE
    resp3 = client.post("/session", json={"sessionId": sid, "connectionId": conn2, "force": True})
