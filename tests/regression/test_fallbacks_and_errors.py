...
def test_model_fallback_succeeds(monkeypatch):
    ...
    import app.services.gemini_helpers
    import app.services.chat_orchestrator
    
    # Inject the exception class so it's available for catching
    app.services.legacy_chat_handler.GeminiError = GeminiError
    app.services.gemini_helpers.GeminiError = GeminiError
    app.services.chat_orchestrator.GeminiError = GeminiError
        ...
        async def agenerate_text(self, *args, **kwargs):
            ...
            for mid in models_to_try:
                if mid == self.primary_model:
                    if log_fallback:
                        log_fallback(mid)
                    last_err = GeminiError("not found", status_code=404)
                    continue
    ...
    monkeypatch.setattr("app.services.gemini_gateway.GeminiGateway", SwitchGateway)
...
def test_upstream_error_maps_to_502_and_sets_cookie(monkeypatch):
    ...
    class ErrorGateway:
        ...
        async def agenerate_text(*args, **kwargs):
            ...
            raise GeminiError("upstream boom", status_code=503)
    ...
    monkeypatch.setattr("app.services.gemini_gateway.GeminiGateway", ErrorGateway)
