...
def initialize_session(
    ...
    *,
    ...
    previous_updated = mem.get("updated") if isinstance(mem, dict) else None
    ...
    if not persona_id and mem:
        persona_meta = (
            mem.get("persona") if isinstance(mem.get("persona"), dict) else {}
        )
        persona_id = persona_meta.get("name") or extract_persona_name_from_text(
            mem.get("character")
        )
        if persona_id:
            logger.info(
                "Recovered personaId '%s' from existing memory for session %s",
                persona_id,
                sid,
            )
    ...
    if persona_id:
        try:
            selected_persona = find_persona(name=persona_id) or find_persona(
        ...
        try:
            ...
            logger.warning(
                "Failed weighted persona selection for session %s: %s", sid, exc
            )
    ...
    if not mem:
        persona_payload = (
            fields["persona"] if selected_persona and fields is not None else None
        )
        mem = {
            "history": [],
            "full_history": [],
    ...
    already_active = _track_connection(
        body, sid, mem, memory_store, logger, previous_updated=previous_updated
    )
    ...
    *,
    ...
    if mem:
        ...
        if body.connectionId in active:
            ...
            memory_store[sid] = mem
            logger.info(
                "Connection deregistered: %s for session %s", body.connectionId, sid
            )
    return {"status": "ok"}
...
def _track_connection(
    ...
    *,
    ...
    updated = previous_updated if previous_updated is not None else mem.get("updated")
    if (
        active_connections
        and isinstance(updated, (int, float))
        and now - updated > STALE_CONNECTION_SECONDS
    ):
        logger.info(
            "Clearing stale active connections for session %s after %.1f seconds: %s",
            sid,
            now - updated,
            active_connections,
        )
    ...
    if body.force:
        logger.info(
            "Force flag detected for session %s. Clearing active connections: %s",
            sid,
            active_connections,
        )
        active_connections = []
    ...
    if body.connectionId:
        logger.debug(
            "Checking connectionId: %s for session %s. Existing: %s",
            body.connectionId,
            sid,
            active_connections,
        )
        if body.connectionId not in active_connections:
            if active_connections:
                logger.info(
                    "Session %s is already active with connections: %s. Replacing with connection: %s",
                    sid,
                    active_connections,
                    body.connectionId,
                )
...
def _session_response(
    _body: Any, sid: str, mem: dict, already_active: bool, initial_card: str | None
) -> dict:
    ...
    return {
        ...
        "initialCard": next(
            (
                ...
                if (m.get("role") or ROLE_ASSISTANT).lower().strip()
                in {ROLE_SYSTEM, ROLE_ASSISTANT}
